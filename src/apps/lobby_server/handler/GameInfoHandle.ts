...
export default class GameInfoHandle {
    ...
    public static async do_req_game_info(session: any, utag: number, proto_type: number, raw_cmd: any){
        let game_info_obj = await RedisGame.get_gameinfo_inredis(utag);
        // Log.info("hcc>>do_req_game_info>>", game_info_obj);
        try {
            if(game_info_obj){
                let userinfostring = JSON.stringify(game_info_obj);
                let body = {
                    status: Response.SUCCESS,
                    userinfostring: userinfostring,
                }
                LobbySendMsg.send(session, LobbyProto.XY_ID.RES_GAMEINFO,utag,proto_type,body);
                return;
            }
            
        } catch (error) {
            Log.error("hcc>>do_req_game_info>>", error);
        }
        LobbySendMsg.send(session, LobbyProto.XY_ID.RES_GAMEINFO, utag, proto_type, {status: Response.ERROR_1});
    }
