...
class GatewayService extends ServiceBase {
	...
	static on_recv_server_player_cmd (session:any, stype:number, ctype:number, utag:number, proto_type:number, raw_cmd:any) {
		...
		if (GatewayHandle.is_login_res_cmd(stype, ctype)) { // 还没登录,utag == session.session_key
			...
			if (body.status == Respones.OK) {
				...
				let prev_session = GatewayHandle.get_session_by_uid(body.uid);
				if (prev_session) {
				...
				if(body.uid){
					...
					GatewayHandle.save_session_with_uid(body.uid, client_session, proto_type);
					body.uid = 0;
					raw_cmd = ProtoManager.encode_cmd(stype, ctype, utag, proto_type, body);
				}
			} 
		}else{ //已经登录,utag == uid
			client_session = GatewayHandle.get_session_by_uid(utag);
		}

		if (client_session){
			NetServer.send_encoded_cmd(client_session,raw_cmd);
			if(ctype == Cmd.eLoginOutRes && stype == Stype.Auth){
				GatewayHandle.clear_session_with_uid(utag);
			}
		}
	}
	...
	static on_player_disconnect(session:any, stype:number) {
		if (stype == Stype.Auth) { // 由Auth服务保存的，那么就由Auth清空
			GatewayHandle.clear_session_with_uid(session.uid);
		}
