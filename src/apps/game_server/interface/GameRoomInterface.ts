...
class GameRoomInterface {
    ...
    static do_player_create_room(utag:number, proto_type:number, raw_cmd:any){
        ...
        if (player.get_user_state() == UserState.MatchIng) {
            player.send_cmd(GameHoodleProto.XY_ID.eCreateRoomRes, { status: Response.INVALIDI_OPT })
        ...
        if (!room) {
            player.send_cmd(GameHoodleProto.XY_ID.eCreateRoomRes, { status: Response.SYSTEM_ERR })
        ...
        player.send_cmd(GameHoodleProto.XY_ID.eCreateRoomRes, { status: Response.OK })
    }
    ...
    static do_player_join_room(utag: number, proto_type: number, raw_cmd: any) {
        ...
        if (player.get_user_state() == UserState.MatchIng) {
            ...
            player.send_cmd(GameHoodleProto.XY_ID.eJoinRoomRes, { status: Response.INVALID_PARAMS })
            return;
        }
        let body = ProtoManager.decode_cmd(proto_type, raw_cmd);
        ...
        if (!room) {
            ...
            player.send_cmd(GameHoodleProto.XY_ID.eJoinRoomRes, { status: Response.SYSTEM_ERR })
            return;
        }
        ...
        if (uroom) {
            ...
            if (room.get_room_id() !== uroom.get_room_id()) {
                ...
                player.send_cmd(GameHoodleProto.XY_ID.eJoinRoomRes, { status: Response.INVALIDI_OPT })
                return;
            }
            //掉线返回房间
            if (room.get_room_id() == uroom.get_room_id()) {
        ...
        player.set_offline(false);
        player.send_cmd(GameHoodleProto.XY_ID.eJoinRoomRes, { status: Response.OK })
    ...
    static do_player_exit_room(utag: number) {
        ...
        if (!room) {
            ...
            player.send_cmd(GameHoodleProto.XY_ID.eExitRoomRes, { status: Response.INVALIDI_OPT })
            return;
        }
        ...
        if (room.get_game_state() != GameState.InView) {
            ...
            GameFunction.broadcast_player_info_in_rooom(room,player);
            player.send_cmd(GameHoodleProto.XY_ID.eExitRoomRes, { status: Response.OK})
            return;
        }
    ...
    static do_player_dessolve_room(utag: number) {
        ...
        if (!room) {
            ...
            player.send_cmd(GameHoodleProto.XY_ID.eDessolveRes, { status: Response.INVALIDI_OPT })
            return;
        }
        ...
        room.broadcast_in_room(GameHoodleProto.XY_ID.eDessolveRes, { status: Response.OK }, player);
        room.kick_all_player();
        player.send_cmd(GameHoodleProto.XY_ID.eDessolveRes, { status: Response.OK })
    }
    ...
    static do_player_get_room_status(utag: number) {
        ...
        let room = roomMgr.get_room_by_uid(player.get_uid())
        if (!room) {
            player.send_cmd(GameHoodleProto.XY_ID.eGetRoomStatusRes, { status: Response.SYSTEM_ERR })
        ...
        player.send_cmd(GameHoodleProto.XY_ID.eGetRoomStatusRes, { status: Response.OK })
    }
    ...
    static do_player_back_room( utag: number) {
        ...
        let room = roomMgr.get_room_by_uid(player.get_uid())
        if (!room) {
            player.send_cmd(GameHoodleProto.XY_ID.eBackRoomRes, { status: Response.INVALIDI_OPT })
