            ...
            otheruids.forEach((uid)=>{
                ...
                if (invitePlayer){
                    ...
                    if(invitePlayer.get_user_state() == UserState.InView && uroom == null){ //空闲状态,没有在其他房间,才能发邀请给他
                        invitePlayer.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainAnswerRes, resBody);
                        player.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainRes, { status: Response.OK });
                    }else{
                        player.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainRes, { status: Response.INVALIDI_OPT });
                    }
                }else{
                    player.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainRes, { status: Response.INVALIDI_OPT });
                }
    ...
    */
    ...
    static do_player_play_again_answer(utag: number, proto_type: number, raw_cmd: any) {//回应玩家utag
        ...
        if (player.get_user_state() != UserState.InView) {//非空闲状态
            player.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainAnswerRes, { status: Response.INVALIDI_OPT });
        ...
        if (body){
            ...
            if (invitePlayer.get_user_state() != UserState.InView){ //非空闲状态
                ...
                invitePlayer.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainAnswerRes, { status: Response.INVALIDI_OPT});
                return;
            }
            ...
            if (invitePlayer) {
                invitePlayer.send_cmd(GameHoodleProto.XY_ID.eUserPlayAgainRes, resBody);
                //玩家同意再次游戏
