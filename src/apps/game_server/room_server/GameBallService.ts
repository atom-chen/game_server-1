...
import GameBallModle from './GameBallModle';
import Log from '../../../utils/Log';

class GameBallService extends ServiceBase {
    service_name: string = "GameBallService"; // 服务名称
    ...
    static on_recv_client_player_cmd(session: any, stype: number, ctype: number, utag: number, proto_type: number, raw_cmd: any) {
        GameBallModle.getInstance().recv_cmd_msg(session, stype, ctype, utag, proto_type, raw_cmd);
    }
...
export default GameBallService;
