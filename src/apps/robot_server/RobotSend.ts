...
class RobotSend {
    ...
    //发给游戏服务,这个服务，是需要自己先连接上的，当前属于客户端
    public static send_game(ctype: number, utag: number, body?: any) {
        let game_server_session = NetClient.get_server_session();
        if (game_server_session) {
            NetClient.send_cmd(game_server_session, Stype.GameHoodle, ctype, utag, ProtoTools.ProtoType.PROTO_BUF, body);
        }
    }
