...
var Room = /** @class */ (function () {
    function Room(roomid, roomdata) {
        ...
        ///////
        this._game_state = State_1["default"].GameState.InView; //游戏状态
    ...
    Room.prototype.get_player_by_seatid = function (seatid) {
        var player_set = this.get_all_player();
    ...
    Room.prototype.born_seatid = function () {
        ...
        var all_seatid = [];
        var all_player = this.get_all_player();
        for (var seatid = 1; seatid <= this._max_player_count; seatid++) {
            all_seatid[seatid] = seatid;
        }
        for (var uid in all_player) {
            var player = all_player[uid];
        ...
        for (var i = 0; i <= exist_seatid.length; i++) {
            ...
            for (var j = 1; j <= this._max_player_count; j++) {
    ...
    Room.prototype.set_game_state = function (game_state) {
        ...
        RedisLobby_1["default"].set_game_state(this.get_room_id(), game_state);
    };
