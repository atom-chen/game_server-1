                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        if (!room) {
                            ...
                            GameSendMsg_1["default"].send(session, GameHoodleProto_1["default"].XY_ID.eCheckLinkGameRes, utag, proto_type, { status: Response_1["default"].ERROR_2 });
                            return [2 /*return*/];
                        }
                        player.send_cmd(GameHoodleProto_1["default"].XY_ID.eCheckLinkGameRes, { status: Response_1["default"].SUCCESS });
                        ...
                        if (room.get_game_state() == State_1["default"].GameState.Gameing) {
                            player.send_cmd(GameHoodleProto_1["default"].XY_ID.eGameStartRes, { status: Response_1["default"].SUCCESS });
                            SendLogicInfo_1["default"].send_player_ball_pos(room, undefined, player);
                            SendLogicInfo_1["default"].send_player_power(room, undefined, player);
                            SendLogicInfo_1["default"].send_player_score(room, undefined, player);
                        }
                        ...
                        if (room.get_game_state() == State_1["default"].GameState.Gameing) {
                            ...
                            for (idx in player_set) {
                                p = player_set[idx];
                                if (p.is_robot()) {
                                    SendLogicInfo_1["default"].send_player_power(room, undefined, p);
                                }
                            }
                        }
                        return [2 /*return*/];
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (!player) {
                            ...
                            GameSendMsg_1["default"].send(session, GameHoodleProto_1["default"].XY_ID.eUserReadyRes, utag, proto_type, { status: Response_1["default"].ERROR_1 });
                            return [2 /*return*/];
                        }
                        userstate = player.get_user_state();
                        if (userstate == State_1["default"].UserState.Ready || userstate == State_1["default"].UserState.Playing) {
                            Log_1["default"].warn(player.get_unick(), "on_user_ready user is already ready or is playing!");
                            player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserReadyRes, { status: Response_1["default"].ERROR_2 });
                            return [2 /*return*/];
                        }
                        return [4 /*yield*/, player.get_room()];
                    case 1:
                        ...
                        if (room) {
                            //已经在游戏中了
                            if (room.get_game_state() == State_1["default"].GameState.Gameing) {
                                player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserReadyRes, { status: Response_1["default"].ERROR_3 });
                                Log_1["default"].warn("on_user_ready error ,game is playing!");
                                return [2 /*return*/];
                            }
                            //已经大结算了
                            if (room.get_cur_play_count() == room.get_max_play_count()) {
                                player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserReadyRes, { status: Response_1["default"].ERROR_4 });
                                Log_1["default"].warn("on_user_ready error ,game is over!");
                                return [2 /*return*/];
                            }
                            //有玩家准备了，发送状态
                            player.set_user_state(State_1["default"].UserState.Ready);
                            SendLogicInfo_1["default"].send_player_state(room, player);
                            is_game_start = GameFunction_1["default"].check_game_start(room);
                            if (is_game_start) {
                                GameFunction_1["default"].set_all_player_state(room, State_1["default"].UserState.Playing);
                                SendLogicInfo_1["default"].broadcast_player_info_in_rooom(room); //刷新局内玩家信息：Playing
                                //设置游戏状态为游戏中
                                room.set_game_state(State_1["default"].GameState.Gameing);
                                //发送游戏开始
                                room.broadcast_in_room(GameHoodleProto_1["default"].XY_ID.eGameStartRes, { status: Response_1["default"].SUCCESS });
                                //游戏逻辑发送
                                SendLogicInfo_1["default"].send_player_first_pos(room);
                                //设置初始权限
                                ...
                                //发送局数
                                SendLogicInfo_1["default"].send_play_count(room);
                            }
                        }
                        return [2 /*return*/];
                }
