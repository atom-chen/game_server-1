...
var GamePlayAgainInterface = /** @class */ (function () {
            ...
            otheruids.forEach(function (uid) {
                ...
                if (invitePlayer) {
                    ...
                    if (invitePlayer.get_user_state() == State_1.UserState.InView && uroom == null) { //空闲状态,没有在其他房间,才能发邀请给他
                        invitePlayer.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainAnswerRes, resBody_1);
                        player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainRes, { status: Response_1["default"].OK });
                    }
                    else {
                        player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainRes, { status: Response_1["default"].INVALIDI_OPT });
                    }
                }
                else {
                    player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainRes, { status: Response_1["default"].INVALIDI_OPT });
    ...
    GamePlayAgainInterface.do_player_play_again_answer = function (utag, proto_type, raw_cmd) {
        ...
        if (player.get_user_state() != State_1.UserState.InView) { //非空闲状态
            player.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainAnswerRes, { status: Response_1["default"].INVALIDI_OPT });
        ...
        if (body) {
            ...
            if (invitePlayer.get_user_state() != State_1.UserState.InView) { //非空闲状态
                ...
                invitePlayer.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainAnswerRes, { status: Response_1["default"].INVALIDI_OPT });
                return;
            }
            ...
            if (invitePlayer) {
                invitePlayer.send_cmd(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainRes, resBody);
                //玩家同意再次游戏
    ...
    GamePlayAgainInterface.player_play_again = function (player_list) {
        ...
        room.broadcast_in_room(GameHoodleProto_1["default"].XY_ID.eUserPlayAgainStartRes, { status: Response_1["default"].OK }); //通知玩家，开始游戏
    };
    //设置房主:room房间
